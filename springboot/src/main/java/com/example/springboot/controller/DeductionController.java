...
public class DeductionController {
    ...
    // 新增或者更新
    @PostMapping
    public Result save(@RequestBody Deduction deduction) {
        deductionService.saveOrUpdate(deduction);
        return Result.success();
    }

    @DeleteMapping("/{id}")
    public Result delete(@PathVariable Integer id) {
        deductionService.removeById(id);
        return Result.success();
    }

    @PostMapping("/del/batch")
    public Result deleteBatch(@RequestBody List<Integer> ids) {
        deductionService.removeByIds(ids);
        return Result.success();
    }

    @GetMapping
    public Result findAll() {
        return Result.success(deductionService.list());
    }
    ...
    public Result findPage(@RequestParam(defaultValue = "") String name,
        ...
        QueryWrapper<Deduction> queryWrapper = new QueryWrapper<>();
        ...
        if (StringUtils.isNotBlank(startDateStr)) {
            queryWrapper.ge("STR_TO_DATE(deduction_date, '%Y-%m-%d')", startDateStr);
        }
        if (StringUtils.isNotBlank(endDateStr)) {
            // 通常不需要加一天，包含时间并且你想包含结束日期当天的所有数据
            queryWrapper.le("STR_TO_DATE(deduction_date, '%Y-%m-%d')", endDateStr);
        }
        ...
        Page<Deduction> page = deductionService.page(new Page<>(pageNum, pageSize), queryWrapper);
        for (Deduction record : page.getRecords()) {
            employeeList.stream()
                    ...
                    .ifPresent(employee -> {
                        ...
                        deductionService.updateById(record);
                    });
        }
    ...
    public Result findPageByDepartmentName(@RequestParam Integer pageNum,
      ...
      /*  if (department == null) {
        ...
        QueryWrapper<Deduction> salaryQueryWrapper = new QueryWrapper<>();
        salaryQueryWrapper.in("employee_id", employeeIds); // 假设Salary实体有employee_id字段
        salaryQueryWrapper.orderByDesc("id"); // 按需排序
        Page<Deduction> salaryPage = deductionService.page(new Page<>(pageNum, pageSize), salaryQueryWrapper);
    ...
    public void export(HttpServletResponse response) throws Exception {
        // 从数据库查询出所有的数据
        List<Deduction> list = deductionService.list();
        // 在内存操作，写出到浏览器
        ...
        String fileName = URLEncoder.encode("Deduction信息表", "UTF-8");
    ...
    public Result imp(MultipartFile file) throws Exception {
        ...
        List<Deduction> list = reader.readAll(Deduction.class);
        deductionService.saveBatch(list);
        return Result.success();
    }
