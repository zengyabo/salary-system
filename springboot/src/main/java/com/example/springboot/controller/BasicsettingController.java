...
public class BasicsettingController {
    ...
    // 新增或者更新
    @PostMapping
    public Result save(@RequestBody Basicsetting basicsetting) {
        basicsettingService.saveOrUpdate(basicsetting);
    ...
    public Result deleteBatch(@RequestBody List<Integer> ids) {
        basicsettingService.removeByIds(ids);
        return Result.success();
    }

    @GetMapping
    public Result findAll() {
        return Result.success(basicsettingService.list());
    }

    @GetMapping("/{id}")
    public Result findOne(@PathVariable Integer id) {
        return Result.success(basicsettingService.getById(id));
    }
    ...
    public Result findPage(@RequestParam(defaultValue = "") String name,
        ...
        QueryWrapper<Basicsetting> queryWrapper = new QueryWrapper<>();
        ...
        if (StringUtils.isNotBlank(startDateStr)) {
            queryWrapper.ge("STR_TO_DATE(basic_date, '%Y-%m-%d')", startDateStr);
        }
        if (StringUtils.isNotBlank(endDateStr)) {
            // 通常不需要加一天，包含时间并且你想包含结束日期当天的所有数据
            queryWrapper.le("STR_TO_DATE(basic_date, '%Y-%m-%d')", endDateStr);
        }
        ...
        Page<Basicsetting> page = basicsettingService.page(new Page<>(pageNum, pageSize), queryWrapper);
        for (Basicsetting record : page.getRecords()) {
            employeeList.stream()
                    ...
                    .ifPresent(employee -> {
                        ...
                        basicsettingService.updateById(record);
                    });
        }
    ...
    public Result findPageByDepartmentName(@RequestParam Integer pageNum,
      ...
      /*  if (department == null) {
        ...
        QueryWrapper<Basicsetting> salaryQueryWrapper = new QueryWrapper<>();
        salaryQueryWrapper.in("employee_id", employeeIds); // 假设Salary实体有employee_id字段
        salaryQueryWrapper.orderByDesc("id"); // 按需排序
        Page<Basicsetting> salaryPage = basicsettingService.page(new Page<>(pageNum, pageSize), salaryQueryWrapper);
    ...
    public void export(HttpServletResponse response) throws Exception {
        ...
        String fileName = URLEncoder.encode("Basicsetting信息表", "UTF-8");
    ...
    public Result imp(MultipartFile file) throws Exception {
        ...
        List<Basicsetting> list = reader.readAll(Basicsetting.class);
        basicsettingService.saveBatch(list);
        return Result.success();
    }
