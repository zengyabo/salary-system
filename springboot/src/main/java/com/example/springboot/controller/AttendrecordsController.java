...
public class AttendrecordsController {
    ...
    public Result findPageByDepartmentName(@RequestParam Integer pageNum,
      ...
      /*  if (department == null) {
        ...
        QueryWrapper<Attendrecords> salaryQueryWrapper = new QueryWrapper<>();
        salaryQueryWrapper.in("employee_id", employeeIds); // 假设Salary实体有employee_id字段
        salaryQueryWrapper.orderByDesc("id"); // 按需排序
        Page<Attendrecords> salaryPage = attendrecordsService.page(new Page<>(pageNum, pageSize), salaryQueryWrapper);
